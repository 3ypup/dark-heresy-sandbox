...
для 3–5 живых игроков, которое можно пройти за 1 игровую сессию (примерно 3–4 часа).

Игроки – аколиты Инквизиции.
Приложение показывает мастеру СЦЕНЫ одну за другой, как страницы квеста.
Поэтому сцены должны быть последовательными и понятными, без лишних деталей.
...

{
  "campaign": {
    ...
    "intro_text": "анонс приключения и вступление, описание обстановки и общей ситуации. Ввод в курс ситуации  для игроков"
  },
  "locations": [
    {
      "name": "название локации",
      "description": "описание обстановки (2–4 предложения)",
  ...
  "npcs": [
    {
      ...
      "description": "как выглядит, что хочет (1–3 предложения)",
      "notes": "секреты и подсказки мастеру (1–3 предложения)"
    }
  ],
  "scenes": [
    {
      ...
      "player_text": "ТО, ЧТО МАСТЕР ЧИТАЕТ ИГРОКАМ. Краткое, образное описание сцены: окружение, состояние персонажей, текущая ситуация и задачи. До 2–3 абзацев.",
      "gm_notes": "секретные заметки мастеру: что на самом деле происходит, какие есть скрытые цели, примеры проверок.",
      "dialogues": [
        { "speaker": "NPC или Игроки", "text": "пример реплики" }
      ],

      "encounter": {
        "objectives": "кратко цель сцены: информация, социальный конфликт или бой. Для боев — условие победы/поражения.",
        "npc_summary": "кто противостоит или с кем взаимодействуют (без чисел и механики).",
        "victory_text": "что происходит при успешном исходе сцены (1–2 предложения).",
        "defeat_text": "что происходит при провале (1–2 предложения).",
        "escape_text": "что происходит если аколиты отступают (1–2 предложения)."
      },

      "choices": [
        {
          "label": "короткое название выбора (на кнопке)",
          "description": "что игроки делают",
          "result_hint": "к чему примерно ведет этот выбор",
...
СТРУКТУРА ПРИКЛЮЧЕНИЯ (СТРОГО):

1) В массиве "scenes" должна быть РОВНО ОДНА сцена с "scene_type": "intro".
   - Это первая сцена.
   - "player_text" — атмосферное вводное (1–3 абзаца): окружение, состояние персонажей, текущее задание.
   - В intro-сцене ОБЯЗАТЕЛЬНО должны быть варианты выбора в "choices", которые ведут дальше.

2) Всего сцен должно быть ОТ 4 ДО 6.
   Пример:
   - 1 intro-сцена,
   - 1–3 social/investigation-сцены (диалоги, расследование),
   - 1–2 combat-сцены,
   - 1 final-сцена.

3) Social / investigation сцены:
   - Это диалоги и расследование.
   - В "gm_notes" обязательно укажи примеры проверок, которые мастер может объявить 
     (например: "Проверка Внимания (-10)", "Запугивание (+10)", "Убеждение (+0)").

4) Combat-сцены:
   - "encounter.objectives" и "npc_summary" описывают врагов и условия победы/поражения.
   - "location_index" ссылается на подходящую локацию и её ascii_map.

5) ДОЛЖНА быть РОВНО ОДНА сцена с "scene_type": "final".
   - Это финальный разговор или бой.
   - У финальной сцены можно не задавать "choices" (или задать минимум).
   - В "gm_notes" опиши варианты финала (успех, частичный успех, провал).
   - В "encounter.victory_text"/"defeat_text"/"escape_text" опиши финальные тексты для мастера,
     а также можно кратко указать пример награды и опыта.

6) ГРАФ СЦЕН:
   - Сцены образуют древовидную или слабо-ветвящуюся структуру без бесконечных циклов.
   - intro-сцена должна вести к другим сценам через "choices".
   - final-сцена НЕ должна иметь "to_scene_index", указывающих дальше.

ОБЩИЕ ТРЕБОВАНИЯ:

- Вся атмосфера, имена и описания строго соответствуют Warhammer 40 000 и Dark Heresy.
- Не используй числовую механику (характеристики, модификаторы) – только художественные описания и подсказки для живого мастера.
- Никакого текста вне JSON. Только один JSON-объект, строго соответствующий схеме выше.
"""
